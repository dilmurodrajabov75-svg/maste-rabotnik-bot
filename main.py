...
# ================= SOZLAMALAR =================
TOKEN = "8350987756:AAFymVPjqNs_XR-M3db7xVMvQBWMP6lDx6Y"
...
def start_command(message):
    ...
    if users_db.get(user_id, {}).get("warnings", 0) >= 3 or users_db.get(user_id, {}).get("blocked"):
        bot.send_message(
            user_id,
...
def send_payment_info(user_id, post_id):
    ...
    if not job_info:
        bot.send_message(user_id, "❌ Ushbu e'lon topilmadi yoki o'chirilgan!")
        return
...
@bot.message_handler(
    content_types=["photo"],
    func=lambda msg: users_db.get(str(msg.from_user.id), {}).get("status") == "waiting_receipt",
)
def handle_receipt(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("❌ Feyk (Ogohlantirish)", callback_data=f"fake_{user_id}"),
    )
...
def admin_callback(call):
    ...
    if action == "approve":
        ...
        bot.send_message(int(target_user_id), success_text, parse_mode="Markdown")
...
# ================= 5. ADMIN ORQALI KANALGA E'LON JOYLAH (BOSQICHMA-BOSQICH) =================
...
def process_step_region(message):
    admin_post_temp[message.chat.id]["region"] = message.text

    # 2-bosqich: Tumani / Aniq manzil
    msg = bot.send_message(
        message.chat.id,
        "🏙 2/6. Tuman yoki aniq manzilni kiriting:\n*(Masalan: Yunusobod tumani / Lokatsiya beriladi)*",
        reply_markup=types.ReplyKeyboardRemove(),
        parse_mode="Markdown",
    )
